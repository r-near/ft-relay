...
use common::{
    allocate_bind_addr, default_faucet_wait, HarnessConfig, TestnetHarness,
};
...
async fn five_k_testnet_test() -> Result<()> {
    env_logger::Builder::from_env(
        env_logger::Env::default()
            .default_filter_or("info,ft_relay=info,near_api=warn"),
    )
    .is_test(true)
    .try_init()
    .ok();
...
async fn run_small_benchmark(harness: &TestnetHarness) -> Result<()> {
    ...
    ensure!(initial_totals.total_tokens == 0, "expected zero initial tokens");
            ...
            tokio::spawn(async move {
                ...
                for i in start_idx..end_idx {
                    ...
                    match client.post(&endpoint).json(&body).send().await {
                        ...
                        Ok(resp) => eprintln!("HTTP {}: {}", resp.status(), resp.text().await.unwrap_or_default()),
    ...
    println!("✅ API requests completed: {}/{} accepted in {:.2}s\n", 
        accepted, SMALL_TEST_REQUESTS, api_duration.as_secs_f64());

    ensure!(accepted >= SMALL_TEST_REQUESTS, 
        "Not all requests accepted: {}/{}", accepted, SMALL_TEST_REQUESTS);
    ...
    for poll in 1..=max_polls {
        ...
        println!("  Poll {}: {} tokens ({:.2}% complete)", 
            poll, totals.total_tokens / YOCTO_PER_TRANSFER, success_rate);
    ...
    println!("  On-chain: {} transfers ({:.2}%)", on_chain_success, success_pct);
    println!("  Status: {}", if success_pct >= 95.0 { "✅ PASSED" } else { "❌ FAILED" });

    ensure!(success_pct >= 95.0, 
        "Success rate too low: {:.2}% (expected ≥95%)", success_pct);
