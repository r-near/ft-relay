...
async fn test_pop_pending_transfers_race_condition() {
    ...
    let race_task = tokio::spawn(async move {
        let mut conn = client_clone.get_multiplexed_async_connection().await.unwrap();
    ...
    let popped = redis_helpers::pop_pending_transfers(&mut conn, token, account_id)
        .await
        .unwrap();
    println!("Main task: pop_pending_transfers returned {} items", popped.len());
    ...
    if lost > 0 {
        ...
        println!("4. Those {} new items were deleted without being returned", lost);
    } else {
